...
## Реалізовано: персональні профілі розрахунку v1

- [x] Версія 1 поточних правил Astroprocessor без зміни попередніх значень управителів.
- [x] Традиційні та сучасні основні управителі; спільне джерело для домів, диспозиторів і підказок.
- [x] Особисті профілі в БД, редагування з контролем редакції, видалення та основний профіль користувача.
- [x] Чернетка профілю в налаштуваннях і вибір профілю на дешборді.
- [x] Знімок правил у натальних і прогнозних результатах, незалежний від змін живого профілю.
- [x] Документація формул та додаткові регресійні сценарії.
- [ ] Запустити нові тести, попередні перевірки точності та браузерні сценарії на тестовій машині.
- [ ] Довільні таблиці управителів та окремі набори аспектів за методом.

Деталі: [Персональні профілі розрахунку v1](CALCULATION_PROFILES_UK.md).

## Реалізовано: розширені фільтри архіву

- [x] Період за датою створення з включенням останнього локального дня.
- [x] Сортування від нових або від давніх зі стабільною курсорною пагінацією.
- [x] Спільні фільтри на дешборді й у drawer, скидання та оновлення без втрати фільтрів.
- [x] Документація та регресійні сценарії для меж періоду й переходів літнього часу.
- [ ] Запустити додані сценарії на тестовій машині.
- [ ] Пошук за людиною та збереження фільтрів між переходами.

## Suggested MVP Sprint Breakdown

### Консультації: перший етап v1

- [x] Документ для власної карти з незалежним знімком джерела.
- [x] Редактор розділів, приватні нотатки та клієнтський перегляд.
- [x] Автозбереження, локальна чернетка вкладки, контроль редакції та повтор запиту.
- [x] Списки на дешборді та для карти, окрема сторінка документа.
- [ ] Запустити тести й перевірити сценарії збереження на тестовій машині.
- [x] Rich-text форматування Tiptap, локальне undo/redo розділу, сумісність із plain-text документами.
- [x] Клієнтський друк / браузерний PDF: вибір розділів, окремий API без приватних нотаток, контроль редакції.
- [ ] Перевірити багатосторінковий друк і PDF у браузері після відновлення Docker.
- [x] Вставлення положень, управління й аспектів зі знімка натальної карти в текст консультації.
- [x] Вставлення подій з архівних прогнозів і транзитів: UTC, соляр/лунар, вибір розділу, джерела й захист від повторів без перерахунку.
- [ ] Браузерна перевірка прогнозних вставок: автозбереження/refresh, помилки мережі, мобільний вигляд, PDF; запуск доданих контрольних сценаріїв.
- [x] Особиста бібліотека текстових шаблонів: категорії, пошук, створення з розділу, форматування, незалежне вставлення, конфлікти редакцій і локальна чернетка.
- [ ] Запуск контрольних сценаріїв бібліотеки шаблонів та браузерна перевірка відновлення, мережевих повторів, двох вкладок і мобільного вигляду.
- [x] Історія версій консультації: транзакційні знімки, приватний перегляд, пагінація, відновлення новою чернеткою з окремим вибором нотаток.
- [ ] Перевірка історії на PostgreSQL та в браузері: rollback, конкурентні збереження, мережевий повтор, legacy-документи, відновлення й мобільний вигляд.
- [x] Порівняння історичної та збереженої консультації за розділами: текст, форматування, порядок, джерела, окремі приватні нотатки.
- [ ] Перевірка порівняння в браузері на мобільному та широкому екрані, запуск доданих контрольних сценаріїв.
- [ ] Підсвічування змінених слів/рядків, контрольні версії та політика строку зберігання історії.
- [x] Натальна карта й окреме колесо соляру в PDF/друці зі збережених знімків, без перерахунку.
- [x] Вибіркові таблиці положень, аспектів між планетами та окремих дат UTC; приватна проєкція й підтвердження відмінних натальних даних.
- [ ] Візуальна перевірка PDF-додатків: дві карти, довгі таблиці A4, кирилиця, мобільний вигляд, помилки завантаження; запуск контрольних сценаріїв.
- [ ] Історія серверних версій, вставлення прогнозних подій, коло карти у клієнтському документі.

Документація: [Консультації](CONSULTATIONS_UK.md).
